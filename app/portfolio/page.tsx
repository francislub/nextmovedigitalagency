    ...
    <main className="min-h-screen bg-background text-foreground font-mono tracking-wide">
      ...
      <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-background via-primary/5 to-background">
        <div className="max-w-4xl mx-auto text-center animate-fadeInUp">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-6 uppercase bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
            Our Portfolio
          </h1>
          <p className="text-xl text-foreground/70 tracking-wide">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap gap-3 justify-center mb-16">
            {categories.map((category, index) => (
              <button
                key={index}
                className={`px-6 py-2.5 rounded-lg font-bold uppercase transition-all duration-300 ${
                  category === 'All'
                    ? 'bg-gradient-to-r from-primary to-secondary text-primary-foreground'
                    : 'bg-card/50 border border-border/50 text-foreground hover:border-primary/30 hover:bg-primary/5'
                }`}
              >
                {category}
              </button>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div
                key={project.id}
                className="group relative bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl overflow-hidden hover:border-primary/30 hover:shadow-lg hover:shadow-primary/20 transition-all duration-300 transform hover:-translate-y-1 hover:scale-105"
              ...
              >
                ...
                <div className="relative w-full h-48 bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center overflow-hidden">
                  <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-40 transition-opacity duration-300 rounded-t-2xl" />
                  <div className="text-center z-10">
                    <p className="text-foreground/50 uppercase tracking-wide text-sm">{project.category}</p>
                  </div>
                </div>

                {/* Content */}
                <div className="p-6">
                  <h3 className="text-xl font-extrabold mb-2 group-hover:text-primary transition-colors line-clamp-2">
                    {project.title}
                  </h3>

                  <p className="text-foreground/70 mb-4 text-sm leading-relaxed tracking-wide">
                    {project.description}
                  </p>
                  ...
                  <div className="bg-primary/10 rounded-lg p-4 mb-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-sm font-bold text-primary">{project.results.increase}</p>
                    </div>
                    <p className="text-sm text-foreground/70 tracking-wide">{project.results.metric}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 text-xs font-bold uppercase bg-border/50 text-foreground/70 rounded-full"
                      >
                        {tag}
                      </span>
                    ))}
